...
\begin{frame}{D\'etermination de $\dot{V}$}
    ...
    \begin{gather*}
        ...
        \intertext{o\`u}
        k = \frac{\text{kts2ms}}{2\cdot \text{fl2m}}
    \end{gather*}
\end{frame}
...
\begin{frame}{Modes lat\'eraux}
    ...
    \[
        ...
        \right.
    \]
    En notant $x = (\psi\; \phi)^\top$, et ayant $p =
    -K\underline{x} + H \psi_c \in \mathbf{R}$, on a
    \begin{gather*}
        \left\lbrace
        \begin{aligned}
            \dot{x} &= Ax + Bu\\
        ...
        \end{bmatrix}
    \end{gather*}
    o\`u $u = \psi_c$, $K = [ k_1 \; k_2 ]$ et $H \in \mathbf{M}_{1\times
    1}(\mathbb{R})$.
\end{frame}

\begin{frame}{Fonction de transfert}
    \[
        C(sI - A)^{-1} =
        \frac{H}{s^2 + k_2 s + \frac{g}{V_E}k_1}
        \begin{bmatrix}
            g/V_E \\ p
        \end{bmatrix}
    \]
    d'o\`u (?)
    \begin{equation*}
        \frac{\psi}{\psi_c} = H \frac{g/V_e}{s^2 + k_2 s + \frac{g}{V_e}k_1}
    \end{equation*}
\end{frame}
